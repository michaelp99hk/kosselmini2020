...
module frame_motor() {
  difference() {
    // No idler cones.
    vertex(45, idler_offset=0, idler_space=100);
    // KOSSEL logotype.
#    translate([22.5, -12, 0]) rotate([90, -90, 30])
      scale([0.11, 0.11, 0.3]) import("logotype.stl");
    // Motor cable paths.
    for (mirror = [-1, 1]) scale([mirror, 1, 1]) {
      // translate([-35, 45, 0]) rotate([0, 0, -30])   cube([4, 15, 15], center=true);
